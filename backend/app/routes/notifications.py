...
from ..email_service import send_email_smtp, get_test_email_html, settings
...
def send_test_email(
    ...
    err_str = None if success else f"SMTP connection error to {settings.EMAIL_HOST}:{settings.EMAIL_PORT}"
...
def retry_notification(
    ...
    if success:
        ...
        noti.status = "failed"
        noti.retry_count += 1
        noti.error_message = f"Retry attempt failed via {settings.EMAIL_HOST}:{settings.EMAIL_PORT}"
